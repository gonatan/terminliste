...
function getChildPicas($parent)
{
    ...
    $db4  = cRegistry::getDb();
    $sql4 = "SELECT LANG.idpica_alloc as id FROM " . $cfg["tab"]["pica_alloc"] . " AS ALLOC JOIN "
        . $cfg["tab"]["pica_lang"]
        . " AS LANG ON ALLOC.idpica_alloc=LANG.idpica_alloc WHERE ALLOC.parentid=$parent AND LANG.online=1";
    $db4->query($sql4);
    if ($db4->numRows() > 0) {
