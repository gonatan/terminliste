...
global $auth, $cfg, $client, $contenido, $db, $errors, $idart, $idartlang, $idcat, $lang;

global $MOD_TE_aliste, $MOD_TE_amodus1, $MOD_TE_amodus2, $MOD_TE_cycle, $MOD_TE_exists, $MOD_TE_highlight;
global $MOD_TE_ort, $MOD_TE_ort_detail, $MOD_TE_ortsliste, $MOD_TE_status, $MOD_TE_teaser, $MOD_TE_termin;
global $MOD_TE_titel, $MOD_TE_xtag, $MOD_TE_zutermine;
...
$cycles = [
    "no"    => "kein Zyklus",
    "tag"   => "täglich",
    "xtag"  => "jeden X-ten Tag",
    "woche" => "wöchentlich",
    "wtage" => "jeden X-ten Wochentag Y",
    "14tag" => "14-tägig",
    "monat" => "monatlich",
    "jahr"  => "jährlich",
];
...
$MOD_TE_tage_abk      = [mi18n("So"), mi18n("Mo"), mi18n("Di"), mi18n("Mi"), mi18n("Do"), mi18n("Fr"), mi18n("Sa")];
...
$MOD_TE_img_show        = "CMS_VALUE[11]" == 'yes';
...
{
    $MOD_TE_termin = []; // Array für die Termininhalte
    $MOD_TE_exists = []; // Array für Prüfung, ob Datensatz bereits existiert
...
if ($_POST["MOD_TE_senden"] == "1") // falls das Terminformular abgeschickt wurde, Daten in Datenbank schreiben
{
    ...
    foreach ($_POST as $key => $value) {
        if (substr($key, 0, 13) == 'cycleweekdays') {
            $collectweekdays .= $value
                . ","; // use coma instead semikolon as an exception, keeps compatibility with functions.terminliste.php
        }
    }
    $MOD_TE_termin[11] =
        ($collectweekdays != "" && $MOD_TE_termin[10] == "wtage") ? substr($collectweekdays, 0, -1) : "";
    ...
    $MOD_TE_termin[12] =
        ($collectweeknums != "" && $MOD_TE_termin[10] == "wtage") ? substr($collectweeknums, 0, -1) : "";
    ...
    if ($MOD_TE_termin[13] != "") {
        $MOD_TE_termin[13] = substr($MOD_TE_termin[13], 0, -1);
    }
    ...
    for ($MOD_TE_zaehler = 4; $MOD_TE_zaehler <= 22; $MOD_TE_zaehler++) {
        ...
        if ($MOD_TE_zaehler != 8 && $MOD_TE_zaehler != 9 && $MOD_TE_zaehler != 14 && $MOD_TE_zaehler != 21) {
            if ($MOD_TE_exists[$MOD_TE_zaehler] == 1) {
                ...
                $MOD_TE_sql .= "VALUES('" . $idartlang . "', '3', '" . $MOD_TE_zaehler . "', '"
                    . $MOD_TE_termin[$MOD_TE_zaehler] . "', '" . $MOD_TE_author . "', '" . $MOD_TE_date . "', '"
                    . $MOD_TE_date . "')";
            }
            $db->query($MOD_TE_sql);
        }
...
if ($MOD_TE_termin[15] == '0') {
    $MOD_TE_termin["status"] = mi18n("frei");
}
if ($MOD_TE_termin[15] == '1') {
    $MOD_TE_termin["status"] = mi18n("reserviert");
}
if ($MOD_TE_termin[15] == '2') {
    $MOD_TE_termin["status"] = mi18n("gebucht");
}
...
if (strlen($MOD_TE_termin[1]) > 1) {
    $MOD_TE_datum1_array = explode("-", $MOD_TE_termin[1]);
}
...
if (!$MOD_TE_termin["cycle"] && $MOD_TE_termin[22] == "") {
    ...
    if ($MOD_TE_termin[2] == '') {
        $MOD_TE_termin[2] = $MOD_TE_termin[1];
    }
    if (!$MOD_TE_termin["cycle"]) {
        $MOD_TE_termin[10] = 'tag';
    }

    $MOD_TE_zdate_array = makedatearray(
        $MOD_TE_termin[0],
        $MOD_TE_termin[2],
        $MOD_TE_termin[10],
        $MOD_TE_termin[16],
        $MOD_TE_termin[17],
        $MOD_TE_checkdate_von,
        $MOD_TE_checkdate_bis,
        $MOD_TE_termin[11],
        $MOD_TE_termin[12],
        $MOD_TE_termin[22]
    );
    if (count($MOD_TE_zdate_array) > 1) {
        for ($MOD_TE_durchlauf2 = 0; $MOD_TE_durchlauf2 < count($MOD_TE_zdate_array); $MOD_TE_durchlauf2++) {
            ...
            if ($MOD_TE_termin[18] == '1' && $MOD_TE_termin[1] < $MOD_TE_today) {
...
$MOD_TE_termin["cycle"]       = !(($MOD_TE_termin[10] == "no"));
...
$tpl->assign(
    'cycle_weekdayselect',
    createCheckbox3($cycleweekdays, explode(",", $MOD_TE_termin[11]), "cycleweekdays", $lang, 105)
);
$tpl->assign(
    'cycle_weeknumselect',
    createCheckbox3($cycleweeknums, explode(",", $MOD_TE_termin[12]), "cycleweeknums", $lang, 50)
);
